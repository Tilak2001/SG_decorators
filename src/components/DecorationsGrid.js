...
const DecorationsGrid = () => {
    return (
        <div className="p-4">
            <h2 className="text-2xl font-bold mb-4">Decorations Available</h2>
            {/* First row: 3 images */}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {decorations.slice(0, 3).map((decoration, index) => (
                    <Link
                        key={index}
                        to={decoration.route}
                        className="relative block overflow-hidden rounded-lg transition-transform duration-300 ease-in-out transform hover:scale-105 hover:shadow-2xl"
                    >
                        <img
                            src={decoration.imageUrl}
                            alt={decoration.title}
